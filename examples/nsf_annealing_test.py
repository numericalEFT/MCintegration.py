...
def generate_model(
    ...
    for mask in masks[::-1]:
        print(mask)
        num_transform_features = sum(mask)
        # flows += [
        ...
        #         num_hidden_channels,
        #         num_bins=num_bins,
        #         #init_width = torch.ones(batchsize, num_transform_features, num_bins),
        #         mask=torch.ones(num_input_channels),
        #     )
        # ]
        flows += [
            nf.flows.CoupledRationalQuadraticSpline(
                ...
                num_bins=num_bins,
                init_width = torch.ones(batchsize, num_transform_features, num_bins),
                mask= mask #torch.ones(num_input_channels),
            )
        ]
    ...
    print("total parameter: ", sum(p.numel() for p in nfm.parameters() if p.requires_grad), "\n")
    return nfm
