import torch
from integrators import MCIntegrator, MCMCIntegrator
...
mc_integrator = MCIntegrator(affine_map, neval=400000, batch_size=1000, device=device)
res = mc_integrator(unit_circle_integrand)
print("Plain MC Integral results:")
print(f"  Integral: {res.mean}")
print(f"  Error: {res.sdev}")

res = MCIntegrator(map_spec, neval=400000, batch_size=1000, device=device)(
